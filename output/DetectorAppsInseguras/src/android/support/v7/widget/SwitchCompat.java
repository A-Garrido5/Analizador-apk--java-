...
public class SwitchCompat extends CompoundButton
{
    ...
    private void animateThumbToCheckedState(final boolean b) {
        ...
        (this.mPositionAnimator = new Animation() {
            ...
            protected void applyTransformation(final float n, final Transformation transformation) {
                SwitchCompat.this.setThumbPosition(mThumbPosition + n * this.val$diff);
            }
        }).setDuration(250L);
    ...
    private void stopDrag(final MotionEvent motionEvent) {
        ...
        this.mTouchMode = 0;
        if (motionEvent.getAction() == (checked ? 1 : 0) && this.isEnabled() && checked) {
    ...
    public void draw(final Canvas canvas) {
        ...
        if (this.mThumbDrawable != null) {
            ...
            final int n3 = n + this.mThumbWidth + mTempRect.right;
            this.mThumbDrawable.setBounds(n2, mSwitchTop, n3, mSwitchBottom);
            final Drawable background = this.getBackground();
            if (background != null) {
                DrawableCompat.setHotspotBounds(background, n2, mSwitchTop, n3, mSwitchBottom);
            }
        }
        super.draw(canvas);
    }
    ...
    protected void onLayout(final boolean b, final int n, final int n2, final int n3, final int n4) {
        ...
        int mSwitchRight;
        if (ViewUtils.isLayoutRtl((View)this)) {
            mSwitchLeft = 0 + this.getPaddingLeft();
            mSwitchRight = 0 + (0 + (mSwitchLeft + this.mSwitchWidth));
        }
        else {
            mSwitchRight = 0 + (this.getWidth() - this.getPaddingRight());
            mSwitchLeft = 0 + (0 + (mSwitchRight - this.mSwitchWidth));
        }
        int paddingTop = 0;
        int mSwitchBottom = 0;
    ...
    public void onMeasure(final int n, final int n2) {
        ...
        final int max = Math.max(this.mSwitchMinWidth, mTempRect.right + (mTempRect.left + 2 * this.mThumbWidth));
    ...
    public void setSwitchTextAppearance(final Context context, final int n) {
        final TypedArray obtainStyledAttributes = context.obtainStyledAttributes(n, SwitchCompat.TEXT_APPEARANCE_ATTRS);
