...
final class FragmentManagerImpl extends FragmentManager implements LayoutInflater$Factory
{
    ...
    public boolean dispatchCreateOptionsMenu(final Menu menu, final MenuInflater menuInflater) {
        ...
        final ArrayList mCreatedMenus2 = this.mCreatedMenus;
        int j = 0;
        ...
        synchronized (this) {
            ...
            if (this.mPendingActions != null) {
                ...
                if (size6 > 0) {
                    ...
                    while (i < size6) {
                        final Runnable runnable = this.mPendingActions.get(i);
                        printWriter.print(s);
                        printWriter.print("  #");
                        printWriter.print(i);
                        printWriter.print(": ");
    ...
    public Fragment findFragmentById(final int n) {
        ...
        if (this.mActive != null) {
            for (int j = -1 + this.mActive.size(); j >= 0; --j) {
                final Fragment fragment = (Fragment)this.mActive.get(j);
                if (fragment != null && fragment.mFragmentId == n) {
                    return fragment;
                }
            }
                ...
                switch (fragment.mState) {
                    case 0: {
                        ...
                        if (fragment.mSavedFragmentState != null) {
                            fragment.mSavedFragmentState.setClassLoader(this.mActivity.getClassLoader());
                    ...
                    if (mState < 2) {
                        ...
                        if (fragment.mView != null && fragment.mContainer != null) {
                            ...
                            if (loadAnimation2 != null) {
                                ...
                                loadAnimation2.setAnimationListener((Animation$AnimationListener)new FragmentManagerImpl$5(this, fragment));
    ...
    public View onCreateView(final String s, final Context context, final AttributeSet set) {
        ...
        if (!Fragment.isSupportFragmentClass((Context)this.mActivity, string)) {
            return null;
        }
        int id;
        if (false) {
        ...
        if (fragment == null) {
            ...
            instantiate.onInflate(this.mActivity, set, instantiate.mSavedFragmentState);
            ...
            if (!fragment.mRetaining) {
                fragment.onInflate(this.mActivity, set, fragment.mSavedFragmentState);
            }
            fragment2 = fragment;
        }
        ...
        if (this.mBackStack != null) {
            if (s == null && n < 0 && (n2 & 0x1) == 0x0) {
                ...
                for (int j = -1 + this.mBackStack.size(); j > n4; --j) {
                    list.add((BackStackRecord)this.mBackStack.remove(j));
                }
                final int n5 = -1 + list.size();
                final SparseArray sparseArray3 = new SparseArray();
                final SparseArray sparseArray4 = new SparseArray();
                for (int k = 0; k <= n5; ++k) {
                    list.get(k).calculateBackFragments(sparseArray3, sparseArray4);
                }
                BackStackRecord$TransitionState backStackRecord$TransitionState = null;
                BackStackRecord$TransitionState popFromBackStack;
                for (int l = 0; l <= n5; ++l, backStackRecord$TransitionState = popFromBackStack) {
                    if (FragmentManagerImpl.DEBUG) {
                        Log.v("FragmentManager", "Popping back stack state: " + list.get(l));
                    }
                    popFromBackStack = list.get(l).popFromBackStack(l == n5, backStackRecord$TransitionState, sparseArray3, sparseArray4);
                }
                this.reportBackStackChanged();
            }
            return true;
    ...
    void restoreAllState(final Parcelable parcelable, final ArrayList list) {
        if (parcelable != null) {
    ...
    ArrayList retainNonConfig() {
        final ArrayList mActive = this.mActive;
                ...
                if (mBackStack != null) {
                    ...
                    if (size3 > 0) {
                        ...
                        for (int k = 0; k < size3; ++k) {
                            mBackStack2[k] = new BackStackState(this, (BackStackRecord)this.mBackStack.get(k));
                            if (FragmentManagerImpl.DEBUG) {
