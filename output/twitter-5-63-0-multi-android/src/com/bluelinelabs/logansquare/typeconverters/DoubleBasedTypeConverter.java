// 
// Decompiled by Procyon v0.5.30
// 

package com.bluelinelabs.logansquare.typeconverters;

import com.fasterxml.jackson.core.JsonGenerator;
import com.fasterxml.jackson.core.JsonParser;

public abstract class DoubleBasedTypeConverter implements TypeConverter
{
    public abstract double convertToDouble(final Object p0);
    
    public abstract Object getFromDouble(final double p0);
    
    @Override
    public Object parse(final JsonParser jsonParser) {
        return this.getFromDouble(jsonParser.p());
    }
    
    @Override
    public void serialize(final Object o, final String s, final boolean b, final JsonGenerator jsonGenerator) {
        jsonGenerator.a(s, this.convertToDouble(o));
    }
}
