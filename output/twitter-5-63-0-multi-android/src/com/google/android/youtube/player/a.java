// 
// Decompiled by Procyon v0.5.30
// 

package com.google.android.youtube.player;

import java.security.cert.CertificateException;
import java.util.Arrays;
import java.io.InputStream;
import java.io.ByteArrayInputStream;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.content.pm.PackageManager$NameNotFoundException;
import com.google.android.youtube.player.internal.bg;
import android.content.Context;
import android.util.Base64;

public final class a
{
    private static final byte[][] a;
    
    static {
        a = new byte[][] { Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0) };
    }
    
    public static YouTubeInitializationResult a(final Context context) {
        try {
            final PackageManager packageManager = context.getPackageManager();
            final String a = bg.a(context);
            if (!a(packageManager.getPackageInfo(a, 64))) {
                return YouTubeInitializationResult.g;
            }
            if (bg.a(context, a)) {
                return YouTubeInitializationResult.e;
            }
            if (!packageManager.getApplicationInfo(a, 0).enabled) {
                return YouTubeInitializationResult.f;
            }
            return YouTubeInitializationResult.a;
        }
        catch (PackageManager$NameNotFoundException ex) {
            return YouTubeInitializationResult.d;
        }
    }
    
    private static boolean a(final PackageInfo packageInfo) {
        try {
            ((X509Certificate)CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(packageInfo.signatures[0].toByteArray()))).checkValidity();
            if (packageInfo.signatures.length == 1) {
                final byte[] byteArray = packageInfo.signatures[0].toByteArray();
                for (int i = 0; i < com.google.android.youtube.player.a.a.length; ++i) {
                    if (Arrays.equals(com.google.android.youtube.player.a.a[i], byteArray)) {
                        return true;
                    }
                }
            }
            return false;
        }
        catch (CertificateException ex) {
            return false;
        }
    }
}
